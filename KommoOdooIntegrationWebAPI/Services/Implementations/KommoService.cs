...
namespace KommoOdooIntegrationWebAPI.Services.Implementations
{
    public class KommoService : IKommoService
    {
        ...
        {
            return await PostAsync("leads", new[] { leadDTO });
        }

        public async Task<string> CreateContactAsync(ContactDTO contactDTO)
        {
            return await PostAsync("contacts", new[] {contactDTO});
        }

        public Task<string> GetLeadsAsync(string with = null, int page = 1, int limit = 250)
        {
            string url = "leads";
        ...
        public Task<string> UpdateLeadAsync(long leadId, LeadDTO leadDTO)
        => PatchAsync($"leads/{leadId}", leadDTO);

        public Task<string> UpdateContactAsync(long contactId, ContactDTO contactDTO)
            => PatchAsync($"contacts/{contactId}", contactDTO);
    }
}
